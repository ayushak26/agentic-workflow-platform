...
import { knowledgeApi, type CollectionResource, type ProfileVersion, type RetrievalPreset, type RetrievalResult } from '../../api/knowledge';
...
export function PlaygroundPage({ onAgents }: { onAgents: () => void }) {
  ...
  useEffect(() => { Promise.all([knowledgeApi.listCollections(), knowledgeApi.listProfiles('retrieval'), knowledgeApi.retrievalPresets()]).then(([cols, profs, values]) => { setCollections(cols); setProfiles(profs); setPresets(values); setCollectionId(cols[0]?.collection_id ?? ''); setProfileId(profs[0]?.profile_id ?? ''); }).catch(error => setError(String(error))); }, []);
