...
export function NodeTypeAskAi({ typeName, onClose }: { typeName: string; onClose: () => void }) {
  ...
  async function ask(q: string) {
    ...
    try {
      const result = await api.askAboutNodeTypes(trimmed, typeName, turns);
    ...
    <div className="fixed inset-0 bg-black/20 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between flex-none">
          <h2 className="text-sm font-semibold text-ink-900">Ask AI — {typeName}</h2>
