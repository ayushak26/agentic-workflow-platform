...
export const FEATURE_HELP: Record<string, FeatureHelpEntry> = {
  ...
  data_mapping: {
    id: 'data_mapping',
    title: 'Inputs',
    description: 'For each field this step needs, choose where its value comes from: type it yourself, or use a value an earlier step already produced.',
    whenToUse: 'Whenever a step\'s field should come from an earlier step\'s result rather than a fixed value.',
    effect: 'Behind the scenes this writes a {{node_id.field}} template reference into the step\'s config — you never type that yourself. Preflight validates that the reference actually exists.',
    example: 'Setting a step\'s "Customer message" field to the "Customer problem" value an earlier "Understand Message" step extracted.',
  },
  variable_picker: {
    id: 'variable_picker',
    title: 'Insert data',
    description: 'The same value picker used everywhere in the Builder, opened here inside a text field — a prompt, an email body, a document template — via "+ Insert data".',
    whenToUse: 'Anywhere you\'re writing free text and need a value from another step mixed into it.',
    effect: 'Inserts the value at the cursor as a chip, e.g. "Customer name · Input", instead of raw {{...}} syntax. It doesn\'t change what the referenced step produces — only how this step reads it.',
    example: 'Inside an email body, picking "Customer name" from the Input step inserts a chip that reads "Hello [Customer name],".',
  },
  node_testing: {
    id: 'node_testing',
    title: 'Node Testing',
  ...
  multi_route: {
    id: 'multi_route',
    title: 'Multi-Route',
    description: 'A Router set to select more than one branch at once — e.g. an enquiry needing Sales AND Engineering AND Supply Chain, instead of exactly one.',
    whenToUse: 'When one evaluation can genuinely touch several branches at the same time, not just one of several alternatives.',
    effect: 'Every selected branch runs in parallel. Selected branches must each finish on their own — nothing downstream may wait for more than one of them, since an unselected branch never runs. Preflight rejects that shape before it can be saved.',
    example: 'A customer enquiry needing pricing and technical validation selects both "sales" and "engineering", and both run — a plain Router could only pick one.',
  },
  workflow_generation: {
