"""OAuth authorization-code flow for Microsoft Graph and Gmail.

Pure, stateless provider functions — no database access here, so they stay
unit-testable against a mocked httpx transport (the same pattern
app/integrations/email/msgraph.py and gmail.py's own tests already use).
app/api/email_oauth.py owns the state/PKCE persistence and orchestration.

Registering the actual Azure AD app (Graph `Mail.Send`/`Mail.ReadWrite`
delegated scopes) and Google Cloud OAuth client (Gmail `gmail.send`/
`gmail.compose`/`gmail.readonly` scopes) is an external, deployment-owner
prerequisite this module cannot perform — it only consumes the client
id/secret such a registration issues (app/config.py's `microsoft_oauth_*`/
`google_oauth_*` settings).
"""
...
_MICROSOFT_SCOPES = "offline_access Mail.Send Mail.ReadWrite User.Read"
_GOOGLE_SCOPES = (
...
def _redirect_uri(provider: Provider) -> str:
    # Must exactly match the path app/api/email_oauth.py's callback route is
    ...
    *,
    ...
    try:
        if provider == "microsoft":
            ...
            if response.status_code >= 400:
                raise OAuthExchangeError(
                    f"microsoft profile lookup returned {response.status_code}: "
                    f"{response.text[:300]}"
                )
            body = response.json()
            ...
            if response.status_code >= 400:
                raise OAuthExchangeError(
                    f"google profile lookup returned {response.status_code}: "
                    f"{response.text[:300]}"
                )
            address = response.json().get("email") or ""
        if not address:
            raise OAuthExchangeError(f"{provider} profile lookup returned no mailbox address")
