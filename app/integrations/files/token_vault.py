"""Encrypted storage for OAuth-issued Google Drive / OneDrive tokens.

Mirrors app/integrations/email/token_vault.py exactly (AES-256-GCM, a
per-process KEK derived from its own master-key setting wrapping a fresh
random per-record DEK, AAD binding the ciphertext to its own connection id)
with its own, distinct KEK — a leaked key must compromise exactly one
security domain, never two by coincidence.

One Mongo collection (`integration_oauth_tokens`), one document per
connection id, holding `{access_token, refresh_token, expires_at}` sealed
together.
"""
...
TOKENS_COLLECTION = "integration_oauth_tokens"
...
def _load_kek() -> bytes:
    ...
    """
    raw = settings.integration_token_vault_master_key
    if not raw.strip() or _is_placeholder(raw) or len(raw.encode("utf-8")) < 32:
        raise VaultKeyMisconfiguredError(
            "integration_token_vault_master_key is missing, a placeholder, "
            "or shorter than 32 bytes"
        )
    if raw == settings.secret_key:
        raise VaultKeyMisconfiguredError(
            "integration_token_vault_master_key must not reuse secret_key — "
            "a leaked JWT signing secret must not also decrypt OAuth tokens"
        )
    if raw == settings.entity_vault_master_key:
        raise VaultKeyMisconfiguredError(
            "integration_token_vault_master_key must not reuse "
            "entity_vault_master_key — key separation across security "
            "domains, same reasoning as secret_key above"
        )
    if raw == settings.email_token_vault_master_key:
        raise VaultKeyMisconfiguredError(
            "integration_token_vault_master_key must not reuse "
            "email_token_vault_master_key — a leaked key must compromise "
            "exactly one OAuth token domain, never both"
        )
    return hashlib.sha256(b"integration-token-vault-kek-v1:" + raw.encode("utf-8")).digest()
