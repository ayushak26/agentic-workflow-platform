"""End-to-end regression tests for workflows/pump_manufacturer_case_routing.yaml.
...
WORKFLOW = load_workflow("workflows/pump_manufacturer_case_routing.yaml")
...
@pytest.mark.asyncio
async def test_an_order_with_no_identified_product_is_not_treated_as_available():
    """With no pump model the availability lookup never runs. Reading its empty
    result as 'not FEASIBLE' sent the order to supply chain; reading it as
    FEASIBLE would be worse still — confirming stock nobody checked."""
    result = await _run(
        {
            "intent": "NEW_ORDER", "customer_name": "Meridian Process Systems",
            "quotation_reference": "QUO-2026-0042", "customer_po_reference": "PO-88213",
    ...
    assert result["status"] == "completed"
    outputs = result["state"]["node_outputs"]
    assert "order_product_not_identified" in outputs
    assert "order_management_ready" not in outputs
